...
func newAdminUserAddCmd(cfg *config.Config, jsonOutput *bool) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			st, authStore, err := openAuthStore(cfg)
			if err != nil {
				return err
			}
			defer st.Close()

			created, err := authStore.CreateAdminUser(cmd.Context(), username, passwordHash, time.Now().UTC())
			if err != nil {
...
func newAdminUserListCmd(cfg *config.Config, jsonOutput *bool) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			users, err := authStore.ListUsers(cmd.Context())
			if err != nil {
				return err
			}
			if *jsonOutput {
...
func newAdminUserSetDisabledCmd(cfg *config.Config, jsonOutput *bool, name, short string, disabled bool) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			updated, err := authStore.SetUserDisabled(cmd.Context(), username, disabled, time.Now().UTC())
			if err != nil {
				return err
			}
			if updated == nil {
...
func newAdminUserDeleteCmd(cfg *config.Config, jsonOutput *bool) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			deleted, err := authStore.DeleteUser(cmd.Context(), username)
			if err != nil {
				return err
			}
			if !deleted {
			...
			if *jsonOutput {
				return writeJSON(map[string]any{"username": username, "deleted": true})
			}
...
func openAuthStore(cfg *config.Config) (*store.Store, store.AuthStore, error) {
	if cfg == nil {
	...
	authStore, ok := any(st).(store.AuthStore)
	if !ok {
		_ = st.Close()
		return nil, nil, fmt.Errorf("auth user provisioning is not supported by this store")
	}
	return st, authStore, nil
}
